...
import { ArrowBothIcon, ArrowLeftIcon, CheckCircleFillIcon, ChevronDownIcon, ChevronLeftIcon, ChevronRightIcon, ChevronUpIcon, ClockIcon, CodeIcon, GraphIcon, LockIcon, PlayIcon, PulseIcon, RepoIcon, SyncIcon, TableIcon } from "@primer/octicons-react"
...
function ParetoSensitivityResults({ sensitivity, detail, setDetail, backToFrontier, goParameters }: { sensitivity: ParetoSensitivityResult; detail: ParetoScenario | null; setDetail: (point: ParetoPoint | ParetoScenario) => void; backToFrontier: () => void; goParameters: () => void }) {
  ...
  return <div className="content-page">
    ...
    <TemporalComparison baseline={sensitivity.baseline} scenarios={sensitivity.scenarios} selected={current} parameter={sensitivity.parameter} onSelectScenario={(scenario) => setDetail(scenario as ParetoScenario)} />
...
function TemporalComparison({ baseline, scenarios, selected, parameter, onSelectScenario }: { baseline: SolveResult; scenarios: TemporalScenario[]; selected: TemporalScenario | null; parameter: string; onSelectScenario: (scenario: TemporalScenario) => void }) {
  ...
  const selectedPosition = scenarios.findIndex((scenario) => scenario.index === selected.index)
  const previousScenario = selectedPosition > 0 ? scenarios[selectedPosition - 1] : null
  const nextScenario = selectedPosition >= 0 && selectedPosition < scenarios.length - 1 ? scenarios[selectedPosition + 1] : null
  ...
  return <section className="github-panel chart-panel temporal-panel">
    ...
    <div className="temporal-controls">
      ...
      <div className="temporal-context"><div className="temporal-context-copy"><span>Selected scenario</span><strong>{selectedLabel}</strong><small>{collapsed}</small></div><div className="temporal-scenario-actions"><IconButton icon={ChevronLeftIcon} aria-label="Select previous scenario" tooltipDirection="n" disabled={!previousScenario} onClick={() => previousScenario && onSelectScenario(previousScenario)} /><IconButton icon={ChevronRightIcon} aria-label="Select next scenario" tooltipDirection="n" disabled={!nextScenario} onClick={() => nextScenario && onSelectScenario(nextScenario)} /></div></div>
    </div>
